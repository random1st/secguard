...
Image tags follow semver: `0.4.1` (exact), `0.4` (latest patch in minor), `latest` (most recent release). The OCI Helm registry mirrors the same scheme. Both are published by `release.yml` on every `v*` tag.
